...
class CachedDVSGestureDataset(Dataset):
    ...
    def _process_and_cache_data(self):
        ...
        for i, (frame, label) in enumerate(tqdm(tonic_dataset, desc=f"Processing {self.split_name} set")):
            ...
            # we need data in format [timesteps, channels, height, width]
...
class OptimizedCNN(nn.Module):
    def __init__(self, num_classes=11):
        ...
        # Feature size after pooling: 256 * 16 * 16
    ...
    def forward(self, x):
        ...
        for t in range(num_steps):
            ...
            features = self.features(x_t)
            
            # Flatten
            features_flat = features.view(batch_size, -1)
            
            # Get FC features (without final classification)
            fc_features = self.classifier[:-1](features_flat)
            
            # Store features
            frame_features.append(fc_features)
        
        # Aggregate features across time (using mean pooling)
        time_aggregated = torch.mean(torch.stack(frame_features, dim=1), dim=1)
        
        # Final classification layer
        output = self.classifier[-1](time_aggregated)
...
# ================= METRICS FUNCTIONS =================
def plot_confusion_matrix(y_true, y_pred, classes=None, epoch=None, save_dir='training_results_DVSGesture_CNN'):
    """
...
def plot_metrics_per_class(results, classes=None, results_dir="training_results_DVSGesture_CNN"):
...
def create_class_performance_dashboard(y_true, y_pred, classes=None, results=None, results_dir="training_results_DVSGesture_CNN"):
    """
    ...
    plt.suptitle('DVS Gesture Recognition Performance (CNN): Class-Level Analysis', fontsize=16)
...
def plot_training_results(results, classes=None):
    ...
    results_dir = "training_results_DVSGesture_CNN"
    if not os.path.exists(results_dir):
        os.makedirs(results_dir)
...
def train_model(model, train_loader, test_loader, criterion, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        # Stop power monitoring and get statistics (without printing)
...
# ================= MAIN FUNCTION =================
def main():
    print("DVS Gesture Classification using CNN with Tonic Data Pipeline")
    ...
    # Try to force MPS to initialize fully
    if device.type == "mps":
    ...
    # Save the model
    model_save_path = os.path.join(results_dir, 'dvs_gesture_cnn_model.pth')
    ...
    print("Training completed. All results saved to 'training_results_DVSGesture_CNN' directory")
